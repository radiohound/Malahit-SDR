...
#define TARGET_NAME_DFU  "DfuDump"

void          print_help( void );
int           hex2bin( unsigned char *obuf, const char *ibuf, int len );
int           check_checksum( unsigned char *inbuf, int len );
unsigned char *ihex2bin_buf( unsigned int *start_address, int *dst_len,
    FILE * inFile, unsigned int max_address );
...
{
  int           i, c, option, vid = 0x0483, pid = 0xdf11, ver = 0xffff;
  char         *tar0, *out_fn, *p, *tar0_lab = NULL;
  FILE         *inFile, *outFile;
  ...
  do
  {
    option = getopt( argc, argv, "hv:p:d:l:c:E:" );
    switch ( option )
    {
  ...
  tar0 = argv[optind++];
  inFile = fopen( tar0, "rt" );
  tar0_buf =
      ihex2bin_buf( &tar0_start_address, &tar0_len, inFile, max_address );

  fclose( inFile );
  if( tar0_buf && ( tar0_len > 0 ) )
  {
    if( ( add_crc32 > 0 )
	&& ( add_crc32 < ( tar0_start_address + tar0_len - 256 ) ) )
    {				// -c request CRC32 placement at given
      // address
      add_crc32 -= tar0_start_address;
      tar0_buf[add_crc32 + 4] = tar0_len >> 0 & 0xFF;	// binary code length 
							// 
      // first(little
      // endian)
      ...
      crc = crc32( 0, tar0_buf, add_crc32 );	// calc CRC upto placement
      // address
      crc =
	  crc32( crc, tar0_buf + add_crc32 + 4,
	  tar0_len - ( add_crc32 + 4 ) );
      // calc the rest of - starting from placement+4 up to end
      tar0_buf[add_crc32] = crc >> 0 & 0xFF;	// CRC placement (little //
						// endian)
    ...
    if( dfu )
    {
      ...
      dfu[5] = 0x01;		// bVersion
      tmp = dfu_len - 0x10;
      dfu[6] = ( unsigned char ) ( tmp & 0xFF );	// DFUImageSize
      // (except Prefix!)
	...
	memmove( dfu + c + 4, TARGET_NAME_DFU, strlen( TARGET_NAME_DFU ) );
      }
      c += 259;
      ...
      // write DFU to file
      out_fn = malloc( 8 + strlen( tar0 ) );
      strcpy( out_fn, tar0 );
      if( ( p = strstr( out_fn, ".hex" ) ) == NULL )
	p = out_fn + strlen( out_fn );
      strcpy( p, ".dfu" );
...
print_help( void )
{
  printf( "STM32 hex2dfu version 1.5\r\n" );
  printf( "Options:\r\n" );
  printf( "\t-c #\t- place CRC32 under this addres\r\n" );
  printf( "\t-d #\t- file version number (default: 0xFFFF)\r\n" );
  printf( "\t-l #\t- Target0 name (default: DfuDump)\r\n" );
  printf( "\t-p #\t- USB Pid (default: 0xDF11)\r\n" );
  printf( "\t-v #\t- USB Vid (default: 0x0483)\r\n" );
  printf( "\t-E #\t- Maximum possible address\r\n" );
  printf( "\t-h\t- help\r\n" );
  printf( "Example: hex2dfu infile.hex\r\n\r\n" );
  exit( 0 );
}

int
hex2bin( unsigned char *obuf, const char *ibuf, int len )
{
  unsigned char c, c2;
...
int
check_checksum( unsigned char *inbuf, int len )
{
...
unsigned char *
ihex2bin_buf( unsigned int *start_address, int *dst_len, FILE * inFile,
    unsigned int max_address )
{
  ...
  *dst_len = 1024 * 1024;
  dst = malloc( *dst_len );	// allocate 129kB of memory for bin data
  // buffer
  if( dst == NULL )
  {
    *dst_len = -2;
  ...
  *start_address = 0;
  while( fgets( oneline, sizeof( oneline ), inFile ) != NULL )
  {
	...
	{			// > Extended Linear Address Record
	  // :020000040803EF
	  // gen new address offset
	  elar =
	      ( unsigned int ) raw[4] << 24 | ( unsigned int ) raw[5] << 16;
	}
	else if( ( raw[0] == 0 ) && ( raw[1] == 0 ) && ( raw[2] == 0 )
	    && ( raw[3] == 1 ) )
	{			// >End Of File record :00000001FF
	  *dst_len = total;	// return total size of bin data && start
	  // address
	  return dst;
	}
	else if( raw[3] == 0 )
	{			// >Data record - process
	  pos =
	      elar +
	      ( ( unsigned int ) raw[1] << 8 | ( unsigned int ) raw[2] );
	  // get start address of this chunk
	  if( start_set == 0 )
	  {
	    *start_address = pos;	// set it as new start addres - only
	    // possible for first data record
	    start_set = 1;	// only once - this is start address of thye
	    // binary data
	  }
	  if( pos >= max_address )
	  {
	  ...
	  {			// enlarge buffer if required
	    // add 8kB of new space
	    unsigned char *dst_new = realloc( dst, *dst_len + 8192 );
  ...
  *dst_len = -3;		// fatal error - no valid intel hex file
  // processed
  free( dst );
  return NULL;
}
